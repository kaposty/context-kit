...
[![Verification](https://img.shields.io/badge/verification-66%20assertions-blue)](#verify-then-prove)
...
Python 3.12 on arm64, 66 of 66 green in each. Both matter for a real reason: `stat -f %m` is
...
Or standalone, if you would rather have the files in your project:

```bash
KIT=path/to/context-kit
mkdir -p .claude/hooks .claude/skills .claude/commands .claude/tools
cp -R "$KIT"/hooks/.  .claude/hooks/
cp -R "$KIT"/skills/. .claude/skills/
cp -R "$KIT"/commands/. .claude/commands/
cp -R "$KIT"/tools/.    .claude/tools/
cp    "$KIT"/.kit-manifest .claude/.kit-manifest
chmod +x .claude/hooks/*.sh .claude/tools/*.sh
```

The last line is the one people skip, and it is the only one that keeps this copy honest
later. `.kit-manifest` is a digest per delivered file, so the installation can tell you it
...
`.claude/settings.json` (merge key by key, do not replace the file). The plugin install
brings its own hook wiring, so it needs only the `env` block.
...
```bash
bash tests/run.sh    # 66 assertions over the renderer, the hooks and the tools
```
...
go red, so the net can actually fail. One of the 66 is aimed at the suite itself: a checker
